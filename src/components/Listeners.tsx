...
const Listeners: React.FC = () => {
  ...
  const [listenerSessions, setListenerSessions] = useState<any[]>([]); // State for listener sessions
  ...
  useEffect(() => {
    fetchListeners();
  }, [sortBy, sortOrder]); // Fetch listeners whenever sortBy or sortOrder changes
  ...
  const fetchListenerDetails = async (listenerId: string) => {
    try {
      ...
      await fetchListenerSessions(listenerId); // Fetch sessions when details are shown
    } catch (error) {
  ...
  const fetchListenerSessions = async (listenerId: string) => {
    try {
      ...
      setListenerSessions(data.sessions); // Set the fetched sessions
    } catch (error) {
  ...
  // Handle form submission for creating or editing a listener
  const handleSubmitListener = async () => {
    ...
    try {
      ...
      const method = selectedListener ? 'PUT' : 'POST'; // Use PUT for editing, POST for creating
      const endpoint = selectedListener ? `${API_URL}/listeners/${selectedListener._id}` : `${API_URL}/listeners`;
      ...
      if (!response.ok) {
        throw new Error(data.message || (selectedListener ? 'Failed to edit listener' : 'Failed to create listener'));
      }
      ...
      alert(`Failed to ${selectedListener ? 'edit' : 'create'} listener. Please try again.`);
    } finally {
  ...
  const resetForm = () => {
    ...
    setFormErrors({});
    setSelectedListener(null); // Reset selected listener for new entries
  };
  ...
  const exportListeners = async () => {
    try {
      ...
      a.href = url;
      a.download = 'listeners_export.csv'; // Set the filename for the download
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          ...
          <button 
            ...
            onClick={() => {
              resetForm(); // Reset form for new listener
              setShowModal(true);
            }}
          >
            <Plus className="h-4 w-4 mr-2" />
      ...
      <div className="mb-6 flex space-x-2">
        ...
        <button
          onClick={() => {
            ...
            fetchListeners(); // Fetch listeners with new sorting
          }}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex space-x-2">
                          ...
                          <button 
                            ...
                            onClick={() => {
                              setNewListener(listener); // Set the selected listener for editing
                              setShowModal(true);
                            }}
                          >
                      ...
                      {listenerSessions.length > 0 ? (
                        listenerSessions.map((session) => (
                          <div key={session._id} className="border rounded p-2 bg-gray-50">
                            ...
                            <p className="text-sm text-gray-700">Time: {new Date(session.time).toLocaleString()}</p>
                          </div>
                        ))
                      ) : (
