...
import { DAYS_OF_WEEK, DEFAULT_TIME_SLOTS, SPECIALTIES, LANGUAGES, GENDERS } from '../constants/listener';
...
const Listeners: React.FC = () => {
  ...
  useEffect(() => {
    const fetchListeners = async () => {
      try {
        setIsLoading(true);
        const data = await listenerService.getListeners();
        setListeners(data);
    ...
    useEffect(() => {
      const filtered = listeners.filter(listener => {
        ...
        const nameMatch = listener.name?.toLowerCase()?.includes(searchTermLower) || false;
        const emailMatch = listener.email?.toLowerCase()?.includes(searchTermLower) || false;
        const specialtyMatch = listener.specialties?.some(specialty => 
    ...
    const handleAddListener = async () => {
      ...
      try {
        // Prepare the listener data according to the API format
        const listenerData = {
        ...
        const response = await listenerService.createListener(listenerData);
        
        // Update the listeners list with the new listener
        setListeners(prev => [...prev, response.listener]);
        setShowModal(false);
        resetForm();
